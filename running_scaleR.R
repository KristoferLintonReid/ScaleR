...
write.csv(results, "results/rna-seq_age_pls.csv")

print("Running Lasso...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=1, nfolds=10)
print("Writing results...")
write.csv(results, "results/rna-seq_age_lasso.csv")

print("Running Ridge...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=0, nfolds=10)
print("Writing results...")
write.csv(results, "results/rna-seq_age_ridge.csv")

print("Running Elastic Nets...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=alpha, nfolds=10)
print("Writing results...")
write.csv(results, "results/rna-seq_age_enets.csv")


print("Starting: MicroArray")
  Micro_Data <- t(read.table(file = 'Data_Pub/HT_HG-U133A', sep = '\t', header = FALSE))
  ...
  results <- ScaleR(x, y, model='pls', gamma=gamma, nfolds=10, ncomp=ncomp)
  print("Writing results...")
  write.csv(results, "results/micro_age_pls.csv")

  print("Running Lasso...")
  results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=1, nfolds=10)
  print("Writing results...")
  write.csv(results, "results/micro_age_lasso.csv")

  print("Running Ridge...")
  results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=0, nfolds=10)
  print("Writing results...")
  write.csv(results, "results/micro_age_ridge.csv")

  print("Running Elastic Nets...")
  results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=alpha, nfolds=10)
  print("Writing results...")
  write.csv(results, "results/micro_age_enets.csv")
}

print("Starting: RPPA")

RPPA.Data <- t(read.table(file = 'Data_Pub/RPPA_RBN', sep = '\t', header = FALSE))
...
results <- ScaleR(x, y, model='pls', gamma=gamma, nfolds=10, ncomp=ncomp)
print("Writing results...")
write.csv(results, "results/rppa_age_pls.csv")

print("Running Lasso...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=1, nfolds=10)
print("Writing results...")
write.csv(results, "results/rppa_age_lasso.csv")

print("Running Ridge...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=0, nfolds=10)
print("Writing results...")
write.csv(results, "results/rppa_age_ridge.csv")

print("Running Elastic Nets...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=alpha, nfolds=10)
print("Writing results...")
write.csv(results, "results/rppa_age_enets.csv")

LCMS.Data <- read.table(file = 'Data_Pub/m_MTBLS2266_LC-MS_positive_reverse-phase_metabolite_profiling_v2_maf.tsv', sep = '\t', header = TRUE)
...
Pheno <- read.delim(file = 'Data_Pub/s_MTBLS2266.txt')
...
results <- ScaleR(x, y, model='pls', gamma=gamma, nfolds=10, ncomp=ncomp)
print("Writing results...")
write.csv(results, "results/lcms_disease_pls.csv")

print("Running Lasso...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=1, nfolds=10)
print("Writing results...")
write.csv(results, "results/lcms_disease_lasso.csv")

print("Running Ridge...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=0, nfolds=10)
print("Writing results...")
write.csv(results, "results/lcms_disease_ridge.csv")

print("Running Elastic Nets...")
results <- ScaleR(x, y, model='glm', gamma=gamma, alpha=alpha, nfolds=10)
print("Writing results...")
write.csv(results, "results/lcms_disease_enets.csv")
